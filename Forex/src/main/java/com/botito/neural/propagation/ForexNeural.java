...
public class ForexNeural {
	...
	public void think(String pathCSV, String file) throws Exception {
		...
		//do {
			...
			hiddenNeurons = Math.round(data[0].length/2) -12;
	...
	public void thinkWithoutTry(String pathCSV, String file) throws Exception {

		int buyOrSell;
		int tryOrder = 0;
		boolean findedOrder = false;
		do {
			//readCSV
			ReadCSV readCSV = new ReadCSV();
			double[][] data = readCSV.readCSV(pathCSV + file);
			...
			System.out.println("Archive: " + file + " prediction: " + actual1);
			
			buyOrSell = Integer.parseInt(actual1);
			
			double e1 = network.calculateError(trainingSet2);
			System.out.println("Archive: " + file + " Network traiined to error: " + e1 );
			...
			if (e1 < 0.20) {
				...
				File file1 = new File(pathCSV + file);
		         ...
		         System.out.println("Archive: " + file  + " discarded by error " + e1);
					
			}
			tryOrder++;
		} while(tryOrder < 5 || findedOrder);	
	}	
	
	public void thinkWithoutTesterTry(String pathCSV, String file) throws Exception {
		lastTest = 18;	
		int buyOrSell;
		int i = 0;
		...
		do {
			...
			double[][] dataTrain = new double[data.length - lastTest][data[0].length]; 
			double[][] idealTrain = new double[data.length - lastTest][1]; 
			double[][] dataTest = new double[1][data[0].length]; 
			double[][] idealTest = new double[1][1]; 
			createArraysTester(
					data,
					dataNormalized,
					dataTrain,
			...
			if (e1 < 0.20) {
				...
				String idealTrainTest = "" + Math.round(idealTest[0][0]); 
				System.out.println("Archive: " + file  +" prediction: " + actual1 +  " actual " + idealTrainTest);
		...
		System.out.println("Archive: " + file  + " prob: " + (assertPrediction/i)*100  + "% efec. cant: " + assertPrediction + " " + i);
		if (e1 < 0.20) {
			WriteOrder writeOrder  =new WriteOrder();
			writeOrder.writeOrder(pathCSV, file, buyOrSell);
		} else {
	...
	private void createArraysTester(
		...
		for (int i = 0; i < data.length - lastTest + 1; i++) {
			if (i < data.length - lastTest) {
				for (int j = 0; j < data[0].length; j++) {
					dataTrain[i][j] = dataNormalized[i][j];
	...
	public final String METHOD_FEEDFORWARD_A = "?:B->SIGMOID->4:B->SIGMOID->?";
	public final String METHOD_FEEDFORWARD_RELU = "?:B->RELU->5:B->LINEAR->?";
	public final String METHOD_BIASLESS_A = "?->SIGMOID->4->SIGMOID->?";
	public final String METHOD_SVMC_A = "?->C->?";
	public final String METHOD_SVMR_A = "?->R->?";
	public final String METHOD_RBF_A = "?->gaussian(c=4)->?";
	public final String METHOD_PNNC_A = "?->C(kernel=gaussian)->?";
	public final String METHOD_PNNR_A = "?->R(kernel=gaussian)->?";
	...
	public void setHiddenNeurons(int hiddenNeurons) {
		ForexNeural.hiddenNeurons = hiddenNeurons;
	}
